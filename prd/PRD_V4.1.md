# DealClaw 数字员工团队 - 产品需求文档 V4.1

> 文档版本: V4.1（Agent-native设计版）  
> 最后更新: 2026-03-30  
> 核心理念: **对话式AI数字团队**，从"描述确定系统"到"定义不确定边界"  
...
| 维度 | V4.0（传统思维） | V4.1（Agent-native思维） |
|------|-----------------|------------------------|
| 设计起点 | 功能描述 | **意图建模** |
| 执行路径 | 预设流程图 | **动态决策树** |
| 异常处理 | 边界外的补丁 | **边界条件为核心** |
...
1. **第3章 Agent核心设计** - 意图建模、决策树、工具地图、边界条件
2. **第10章 长期记忆机制** - 记忆类型、权重策略、检索召回
3. **第11章 评估指标体系** - Agent-native评估指标
4. **附录：Agent自检清单** - 三问验证

### 关联文档

- `agent-design/intent-modeling.md` - 意图建模详细设计
- `agent-design/decision-tree.md` - 决策树详细设计
- `agent-design/tool-map.md` - 工具地图详细设计
- `agent-design/boundary-conditions.md` - 边界条件详细设计
- `agent-design/memory-system.md` - 记忆机制详细设计
- `agent-design/evaluation-metrics.md` - 评估指标详细设计  
...
![DealClaw 产品主界面](images/product-ui-main.jpg)
...
| **V4.1** | **2026-03-30** | **Agent-native设计版，核心思维转变：从"描述确定系统"到"定义不确定边界"；新增第3章Agent核心设计（意图建模/决策树/工具地图/边界条件）、第10章长期记忆机制、第11章评估指标体系；新增附录自检清单** |

---

## 3. Agent核心设计

> **核心思维转变**: 从"描述确定系统"到"定义不确定边界"
> 
> 传统PRD回答"这个功能是什么"，Agent-native PRD回答"Agent在什么情况下应该停下来"
...
| 意图类别 | 核心意图 | 置信度阈值 | 风险等级 |
|---------|---------|-----------|---------|
| 任务管理 | 创建获客任务 | 0.75/0.85 | 低 |
| 任务管理 | 查询任务进度 | 0.70 | 低 |
| 任务管理 | **删除任务** | 0.90 | **高** |
| 数据查询 | 获取新线索 | 0.70 | 低 |
| 知识管理 | **更新企业画像** | 0.85 | **高** |
| 执行操作 | **发送邮件** | 0.90 | **高** |
| 执行操作 | **批量发送>100封** | 0.95 | **极高** |

#### 3.1.3 模糊意图处理策略

| 模糊类型 | 识别特征 | 处理策略 |
|---------|---------|---------|
| 代词指代不明 | "帮我优化它" | 上下文回溯+选项确认 |
| 动作对象模糊 | "发送出去" | 确认目标列表 |
| 程度副词模糊 | "增加一些" | 量化确认（约15%） |
| 时间表达模糊 | "最近的效果" | 时间范围选项 |
| 多任务混淆 | "查看并优化" | 分解为子任务 |
...
详细设计参考: `agent-design/intent-modeling.md`
...
| 决策类型 | 判定条件 | Agent行为 | 用户反馈 |
|---------|---------|----------|---------|
| **继续执行** | 置信度>0.85 + 完整度>0.8 | 继续规划 | 展示进度 |
| **暂停询问** | 置信度0.6-0.85 | 暂停提问 | 展示选项 |
| **推测确认** | 置信度>0.7 + 有歧义 | 展示推测 | "您是想...对吗？" |
| **终止任务** | 无法完成/用户取消 | 终止并报错 | 说明原因 |
| **降级处理** | 工具失败 | 使用替代方案 | 说明降级原因 |

#### 3.2.2 关键决策阈值

| 阈值名称 | 默认值 | 说明 |
|---------|-------|------|
| 直接执行置信度 | 0.85 | 高于此值直接执行 |
| 推测确认置信度 | 0.70 | 此区间需要推测确认 |
| 请求澄清置信度 | 0.60 | 低于此值请求澄清 |
| 信息完整度阈值 | 0.80 | 高于此值认为信息完整 |
| 风险操作置信度 | 0.90 | 风险操作需更高置信度 |

详细设计参考: `agent-design/decision-tree.md`
...
详细设计参考: `agent-design/tool-map.md`
...
|-------|---------|
| 0.9-1.0 | 直接回答 |
| 0.8-0.9 | 正常回答+"如有疑问请告诉我" |
| 0.7-0.8 | "根据现有信息，我认为..." |
| 0.6-0.7 | "这个我不太确定，但推测可能是..." |
| <0.6 | 请求澄清 |
...
详细设计参考: `agent-design/boundary-conditions.md`

---

## 4. 核心概念

> 注：原第3章调整为第4章，以下保持V4.0内容
...
| 记忆类型 | 内容 | 存储位置 | 有效期 |
|---------|------|---------|-------|
| 企业画像 | 产品/优势/市场/客户 | 关系数据库 | 长期 |
| 用户偏好 | 沟通风格/决策习惯 | 关系数据库 | 长期 |
| 任务历史 | 过往任务及效果 | 时序数据库 | 2年 |
| 对话历史 | 完整对话记录 | 向量数据库 | 90天 |
| 上下文窗口 | 当前对话最近20轮 | 内存 | 会话级 |

### 10.2 记忆权重策略

**权重计算公式**:
```
weight = base_weight × recency_factor × frequency_factor × confidence_factor

base_weight: 记忆类型基础权重
  - 企业画像: 1.0
  - 用户偏好: 0.9
  - 最近任务: 0.8
  - 历史任务: 0.6

recency_factor: 时效性衰减（指数衰减，半衰期70天）
frequency_factor: 访问频率加成
confidence_factor: 置信度调整
```
...
| Agent推断 | 标记为推断，置信度0.7 | 新版本+待确认 |
| 用户纠正 | 更新+提升权重 | 新版本+纠正标记 |
| 自动归档 | weight<0.1且30天未访问 | 冷存储 |

详细设计参考: `agent-design/memory-system.md`

---

## 9. 评估指标体系

> **核心理念**: Agent-native产品的专属评估体系

### 11.1 核心指标

| 指标 | 定义 | 目标值 | 评估方法 |
|-----|------|-------|---------|
| **任务完成率** | Agent独立完成任务比例 | >85% | 抽样100个任务 |
| **人工干预率** | 触发"暂停询问"频率 | 15-25% | 每100次对话 |
| **意图识别准确率** | 正确识别意图比例 | >90% | 标注样本测试 |
| **用户接受率** | 用户采纳Agent输出比例 | >80% | 用户反馈 |
| **安全边界触发率** | 风险确认触发频率 | 10-20% | 监控日志 |
| **平均对话轮数** | 完成任务平均轮数 | <5轮 | 对话统计 |
| **工具降级率** | 工具失败触发降级比例 | <10% | 工具调用日志 |
...
详细设计参考: `agent-design/evaluation-metrics.md`
...
|-----|------|
| **作为** | 外贸业务员 |
| **我希望** | 看到「画像确认卡片」并能直接编辑确认 |
| **以便** | 确保画像准确 |
| **优先级** | P0 |
| **涉及Agent** | 主管Agent |

**故事详述**：
主管Agent根据用户输入和追问答案，生成「客户画像卡片」，以可视化方式展示AI理解的目标客户特征。用户可以一键编辑任何字段，确认后画像锁定并用于后续任务执行。
...
| 分支点 | 条件 | 流程 |
|-------|------|------|
| B1 | 编辑单个字段 | 弹出字段编辑器，保存后更新卡片 |
| B2 | 整体编辑 | 进入全表单编辑模式 |
| B3 | 重新填写 | 清空画像，重新询问 |
| B4 | 置信度低（<70%） | 提示"信息不够完整，建议补充" |

**异常流程**

| 异常 | 触发条件 | 系统处理 | 用户反馈 |
|-----|---------|---------|---------|
| E1 | 必填字段缺失 | 高亮缺失字段，禁用确认按钮 | "请完善标红字段" |
| E2 | 数据格式错误 | 实时校验，提示正确格式 | 输入框红色边框+提示 |

---

#### 5.4.5 US-009: 获客方案确认与执行
...
|-----|------|
| **作为** | 外贸业务员 |
| **我希望** | 看到「获客方案卡片」展示各Agent任务分配和预期效果 |
| **以便** | 确认或调整方案后执行 |
| **优先级** | P0 |
| **涉及Agent** | 主管Agent、SEO Agent、Email Agent |
...
│  │ 🔍 SEO Agent 策略                                      [配置⚙️]    │   │
│  │ ├─ 模式：[AI建站 ▼]                                               │   │
│  │ ├─ 目标关键词：wholesale camping gear, bulk outdoor equipment     │   │
│  │ ├─ 内容频率：[每周2篇 ▼]                                           │   │
│  │ ├─ 文章类型：[行业趋势 ▼]                                          │   │
│  │ └─ 预期线索：[8条/月]                                              │   │
│  │                                                                     │   │
│  │ ✉️ Email Agent 策略                                    [配置⚙️]    │   │
│  │ ├─ 发送渠道：[平台代发 ▼]                                          │   │
│  │ ├─ 目标客户：800家（已筛选）                                        │   │
│  │ ├─ 发送频率：[每日30封 ▼]                                          │   │
│  │ ├─ 发送时段：[美国时间9-11点 ▼]                                     │   │
...
| 步骤 | 动作 | 系统响应 | 输出 |
|-----|------|---------|------|
| 1 | 用户确认方案 | 验证配置完整性 | 任务启动 |
| 2 | 分配SEO任务 | SEO Agent接收任务 | 开始执行 |
| 3 | 分配Email任务 | Email Agent接收任务 | 开始执行 |
| 4 | 汇总启动状态 | 展示执行确认 | 任务进行中 |

**分支流程**

| 分支点 | 条件 | 流程 |
|-------|------|------|
| B1 | 调整SEO配置 | 展开SEO配置面板，修改后实时更新预期效果 |
| B2 | 调整Email配置 | 展开Email配置面板，修改后重新计算客户数 |
| B3 | 启用WhatsApp | 展开WhatsApp配置，增加渠道后更新整体预期 |
| B4 | 单渠道执行 | 只启动选中的Agent，另一标记为"跳过" |

**异常流程**

| 异常 | 触发条件 | 系统处理 | 用户反馈 |
|-----|---------|---------|---------|
| E1 | 预算超限 | 提示"当前配置预估成本超出预算" | 提供优化建议 |
| E2 | 配置冲突 | 检测并提示冲突项 | "SEO和Email目标客户地区不一致" |
| E3 | 站点未连接 | 禁用SEO发布功能 | "请先连接独立站" |

---

### 5.5 SEO Agent详细用户故事

#### 5.5.1 US-010: 编辑与发布SEO文章
...
│  │ 标题：                                                              │   │
│  │ ┌─────────────────────────────────────────────────────────────────┐│   │
│  │ │ 2024年露营装备市场趋势：批发商如何把握机遇                        ││   │
│  │ └─────────────────────────────────────────────────────────────────┘│   │
│  │                                                                     │   │
│  │ 正文：                                                              │   │
│  │ ┌─────────────────────────────────────────────────────────────────┐│   │
│  │ │ The camping gear market is experiencing unprecedented growth... ││   │
│  │ │                                                                 ││   │
│  │ │ For wholesalers looking to expand their outdoor product...      ││   │
│  │ │                                                                 ││   │
│  │ │ Key trends include sustainable materials, lightweight...        ││   │
│  │ │                                                                 ││   │
│  │ │ [编辑中... 支持富文本格式、图片插入、链接添加]                    ││   │
│  │ └─────────────────────────────────────────────────────────────────┘│   │
│  │                                                                     │   │
│  │ ┌─────────────────────────────────────────────────────────────────┐│   │
│  │ │ 📊 实时SEO建议                                                   ││   │
...
| 分支点 | 条件 | 流程 |
|-------|------|------|
| B1 | 质量检测未通过 | 高亮问题段落，提供修改建议 |
| B2 | 定时发布 | 选择发布时间，加入队列 |
| B3 | 保存草稿退出 | 保存当前进度，提示"已保存" |
| B4 | 预览模式 | 展示文章在独立站的实际效果 |

**异常流程**

| 异常 | 触发条件 | 系统处理 | 用户反馈 |
|-----|---------|---------|---------|
| E1 | 发布失败 | 保存草稿，提示失败原因 | "发布失败：站点连接超时" |
| E2 | 内容违规 | 检测敏感词，禁止发布 | "内容包含敏感信息，请修改" |
| E3 | 频繁保存 | 防抖处理 | 静默保存，不打扰用户 |

---

### 5.6 Email Agent详细用户故事

#### 5.6.1 US-011: 预览邮件与发送策略设置
...
│  │ │                                                                │  │   │
│  │ │ □  From                  Subject                    Time      │  │   │
│  │ │ ⭐  Sarah Chen      Quality Camping Gear...     9:30 AM      │  │   │
│  │ │    sarah@yourcompany.com                                      │  │   │
│  │ │    Hi Mike, I noticed Outdoor Gear Co...                [●●●]│  │   │
│  │ │                                                                │  │   │
│  │ └───────────────────────────────────────────────────────────────┘  │   │
│  │                                                                     │   │
│  │ 💡 专业度评分：92/100                                               │   │
│  │ ✓ 发件人显示正常  ✓ 主题行吸引  ✓ 预览文本完整  ✓ 无垃圾关键词      │   │
│  │                                                                     │   │
│  │ ⚠️  建议优化：主题行可增加紧迫感（如"Limited Time"）                  │   │
│  │                                                                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│  │                                                                     │   │
│  │ 📊 发送频率：                                                        │   │
│  │ [冷启动期：10封/天 ▼] → [成长期：50封/天 ▼] → [成熟期：100封/天 ▼]   │   │
│  │                                                                     │   │
│  │ 🕐 发送时间（美国时区）：                                             │   │
│  │ [周二 ▼] [周四 ▼]                    [9:00-11:00 ▼]                │   │
│  │                                                                     │   │
│  │ 📨 发送渠道：[平台代发（推荐）▼]                                      │   │
│  │                                                                     │   │
│  │ 🔄 自动跟进：[开启 ⭕]  3天后无回复自动发送跟进邮件                    │   │
│  │                                                                     │   │
│  │ ┌─────────────────────────────────────────────────────────────────┐│   │
│  │ │ 📅 发送日历预览                                                  ││   │
│  │ │ 4月1日(二): 10封   4月2日(三): 10封   4月3日(四): 10封         ││   │
│  │ │ 4月4日(五): 停发   4月5日(六): 停发   4月6日(日): 停发         ││   │
│  │ │ 4月7日(一): 10封   4月8日(二): 30封   4月9日(三): 30封...    ││   │
│  │ └─────────────────────────────────────────────────────────────────┘│   │
│  │                                                                     │   │
│  │ 💡 策略建议：周二/周四上午打开率最高，建议保持                        │   │
│  │                                                                     │   │
│  │              [取消]  [🔄 重置默认]  [💾 保存策略]                   │   │
│  │                                                                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
...
| 步骤 | 动作 | 系统响应 | 输出 |
|-----|------|---------|------|
| 1 | 请求邮件预览 | 生成邮件HTML | 预览视图 |
| 2 | 分析专业度 | 评分并给出建议 | 优化建议 |
| 3 | 配置发送策略 | 计算发送计划 | 发送日历 |
| 4 | 保存策略 | 存储配置 | 确认消息 |
...
| 分支点 | 条件 | 流程 |
|-------|------|------|
| B1 | 选择自有邮箱 | 切换至SMTP配置，显示域名信誉评分 |
| B2 | 调整发送频率 | 实时更新完成日期和发送日历 |
| B3 | 开启A/B测试 | 增加测试组配置（主题行/内容变体） |
| B4 | 设置黑名单 | 排除特定域名/邮箱后缀 |

**异常流程**

| 异常 | 触发条件 | 系统处理 | 用户反馈 |
|-----|---------|---------|---------|
| E1 | 发送频率过高 | 提示"超过域名信誉承载能力" | 建议降低频率 |
| E2 | 发送时间不合适 | 提示"该时段打开率较低" | 推荐最佳时段 |
| E3 | 退信率预测高 | 分析原因（邮箱质量/内容问题） | 提供优化建议 |

---

### 5.7 数据与线索中心用户故事

#### 5.7.1 US-012: 线索收件箱管理
...
│  │ ─────────────────────────────────────────────────────────────────  │   │
│  │                                                                     │   │
│  │ 🔥 筛选：[全部 ▼] [渠道：全部 ▼] [评分：80+ ▼] [时间：本月 ▼]      │   │
│  │                                                                     │   │
│  │ ┌─────────────────────────────────────────────────────────────────┐│   │
│  │ │ ⭐ NEW    Summit Camping Supply                评分：92  2小时前││   │
│  │ │         📧 Email    📍 Austin, TX    🏭 户外零售    [查看]    ││   │
│  │ ├─────────────────────────────────────────────────────────────────┤│   │
│  │ │ ⭐ NEW    Mountain Gear Wholesale              评分：88  5小时前││   │
│  │ │         🔍 SEO      📍 Denver, CO    🏭 装备批发    [查看]    ││   │
│  │ ├─────────────────────────────────────────────────────────────────┤│   │
│  │ │         Outdoor Adventure Co.                  评分：76  1天前 ││   │
│  │ │         📧 Email    📍 Seattle, WA   🏭 户外品牌    [查看]    ││   │
│  │ ├─────────────────────────────────────────────────────────────────┤│   │
│  │ │         Trail Blazer Inc.                      评分：72  2天前 ││   │
│  │ │         💬 WhatsApp 📍 Portland, OR  🏭 零售商      [查看]    ││   │
│  │ └─────────────────────────────────────────────────────────────────┘│   │
│  │                                                                     │   │
│  │ ┌─────────────────────────────────────────────────────────────────┐│   │
│  │ │ 📊 线索统计                                                      ││   │
│  │ │ 本月新增：23条  │  高意向：8条  │  已跟进：15条  │  待跟进：8条 ││   │
│  │ │                                                                 ││   │
│  │ │ 渠道分布：📧 Email 65%  │  🔍 SEO 25%  │  💬 WhatsApp 10%   ││   │
│  │ └─────────────────────────────────────────────────────────────────┘│   │
│  │                                                                     │   │
│  │              [📥 导出Excel]  [🏷️ 批量标签]  [👤 分配]              │   │
│  │                                                                     │   │
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ 🎯 线索详情 - Summit Camping Supply                                │   │
│  │ ─────────────────────────────────────────────────────────────────  │   │
│  │                                                                     │   │
│  │ 🏢 Summit Camping Supply                                          │   │
│  │ 📍 Austin, Texas, USA    🏭 户外用品零售    👥 85人               │   │
│  │ 💰 年采购额：$200万-500万    📧 mike@summitcamping.com            │   │
│  │                                                                     │   │
│  │ ⭐ AI评分：92/100                                                  │   │
│  │ 依据：打开了3封邮件 │ 点击了网站链接 │ 回复询问价格 │ 规模匹配      │   │
│  │                                                                     │   │
│  │ 💬 互动历史：                                                       │   │
│  │ • 3/27 9:30 - 发送开发信（已送达）                                   │   │
│  │ • 3/27 14:20 - 打开邮件                                            │   │
...
│  │ 🏷️ 标签：[高意向] [已回复] [待报价]                                 │   │
│  │                                                                     │   │
│  │ 💬 最新消息："Can you send me your wholesale price list..."         │   │
│  │                                                                     │   │
│  │ [💬 生成回复]  [📧 查看邮件]  [📝 添加备注]  [👤 分配]             │   │
│  │ [🏷️ 编辑标签]  [📊 查看公司]  [❌ 标记无效]                        │   │
│  │                                                                     │   │
...
| 分支点 | 条件 | 流程 |
|-------|------|------|
| B1 | 批量操作 | 多选线索，批量标签/分配/导出 |
| B2 | 分配给同事 | 选择团队成员，转移线索所有权 |
| B3 | 添加自定义标签 | 创建新标签，标记线索 |
| B4 | 搜索线索 | 按公司名/地区/联系人搜索 |

**异常流程**

| 异常 | 触发条件 | 系统处理 | 用户反馈 |
|-----|---------|---------|---------|
| E1 | 线索数据加载慢 | 先展示骨架屏，渐进加载 | "加载中..." |
| E2 | 线索已被其他同事跟进 | 显示跟进人信息 | "该线索已由小王跟进" |
| E3 | 公司信息缺失 | 提示"信息不完整" | 建议补充调研 |
...
| **意图置信度** | Agent对意图识别结果的确定程度（0-1） |
| **信息完整度** | 必填槽位填充比例（0-1） |
| **决策树** | Agent根据条件自主决策继续/暂停/终止/降级的逻辑 |
| **边界条件** | Agent需要停下来询问或确认的场景定义 |
| **长期记忆** | Agent跨会话保留的企业知识、用户偏好、任务历史 |
| **人工干预率** | 需要Agent停下来询问用户的对话比例 |

### 13.2 Agent自检清单（开发前必答）

基于Agent-native设计原则，开发前必须回答以下三问：

#### 问题1: 这个Agent在什么情况下应该停下来？

| 场景 | 停下条件 | 处理方式 |
|-----|---------|---------|
| 意图不明确 | 置信度 < 0.6 | 请求澄清："您是想...？" |
| 信息缺失 | 必填槽位缺失 | 追问："请告诉我..." |
| 存在歧义 | 多意图可能 > 0.3 | 推测确认："您是想A还是B？" |
| 工具失败 | 主路径失败且无降级 | 暂停报错："遇到技术问题..." |
| 风险操作 | 删除/发送/修改画像 | 等待确认：预览+确认按钮 |
| 用户暂停 | 用户明确说"暂停"/"等一下" | 立即停止，保存状态 |
| 超出能力 | 用户请求超出产品范围 | 坦诚告知："我目前还不能..." |

**检查标准**: 每个核心流程都定义了明确的"停下"条件

#### 问题2: 如果它做了一个错误的决策，用户能发现吗？

| 机制 | 实现方式 |
|-----|---------|
| 置信度标识 | 所有输出附带置信度，<0.8明确标注 |
| 推理透明 | 关键决策提供依据："基于...我认为..." |
| 结果可查看 | 画像/方案/内容以卡片展示，可审阅 |
| 结果可编辑 | 所有生成内容支持编辑修改 |
| 执行前确认 | 风险操作前有预览环节 |
| 支持回退 | 可回退到上一确认节点 |
| 用户纠正 | 用户纠正后立即更新并道歉 |

**检查标准**: 错误决策可被用户发现、纠正、恢复

#### 问题3: 有没有哪个动作，是绝对不能在没有人工确认的情况下执行的？

| 风险操作 | 确认级别 | 确认方式 | 后果说明 |
|---------|---------|---------|---------|
| **发送邮件** | 单层确认 | 预览+确认按钮 | 不可逆，影响客户印象 |
| **批量发送>100封** | **双重确认** | 验证码+文字确认 | 高风险，域名信誉 |
| **删除进行中任务** | **双重确认** | 预览+"删除"文字输入 | 永久删除，数据丢失 |
| **修改企业画像** | 单层确认 | 变更对比+确认 | 影响后续所有任务 |
| **发布文章到站点** | 单层确认 | 预览+确认 | 公开可见，SEO影响 |

**检查标准**: 每个风险操作都有对应的确认机制
...
| 文档 | 路径 | 内容 |
|-----|------|------|
| 意图建模详细设计 | `agent-design/intent-modeling.md` | 意图分类、实体提取、模糊处理 |
| 决策树详细设计 | `agent-design/decision-tree.md` | 状态机、决策条件、多轮流程 |
| 工具地图详细设计 | `agent-design/tool-map.md` | 工具定义、失败降级、调用监控 |
| 边界条件详细设计 | `agent-design/boundary-conditions.md` | 五大边界、处理策略、确认机制 |
| 记忆机制详细设计 | `agent-design/memory-system.md` | 记忆类型、权重策略、检索召回 |
| 评估指标详细设计 | `agent-design/evaluation-metrics.md` | 指标体系、监控告警、优化机制 |
